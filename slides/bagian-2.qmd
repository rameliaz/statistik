...
* Ide dasarnya adalah menyusun persamaan garis yang dapat digunakan untuk **memperkirakan** nilai Y ketika nilai X diketahui
  - Contohnya, kita memiliki hipotesis bahwa **kemandirian** berkorelasi dengan **neuroticism** (negatif) dan **trust** (positif) dan memang menemukan korelasi antara keduanya
...
* Ternyata, _neuroticism_ berkorelasi positif dengan tingkat kemandirian.
  - Ini berbeda dengan hipotesis Marimar sebelumnya.
...
* Artinya, efek langsung satu prediktor terhadap _outcome_, tanpa mempertimbangkan nilai prediktor lain dalam model (prediktor lain di-_hold constant_ = 0)
* Tetapi, **dalam model regresi dengan suku interaksi**, _main effect_ merupakan efek kondisional pada nilai tertentu dari moderator, yaitu rata-ratanya setelah _centering_
* Contoh: ketika _main effect_ _neuroticism_ (_b_) = 0.31, maka _b_ adalah efek _neuroticism_ terhadap kemandirian ketika _trust in organismic development_ = rata-rata

**_Interaction effect_**

* Efek interaksi adalah interpretasi substantif dari koefisien (_slope_) suku interaksi, yaitu seberapa besar efek X~1~ terhadap _outcome_ berubah setiap satu unit perubahan X~2~
* Jika signifikan, artinya hubungan antara X~1~ dan _outcome_ tidak konstan, melainkan berbeda-beda tergantung nilai X~2~
...
* Ketika melakukan analisis regresi dengan suku interaksi, maka kita perlu melakukan prosedur _mean centering_ untuk prediktor dan moderator.

* Alasannya, koefisien _main effect_ akan lebih mudah diinterpretasi
  - Tanpa _centering_, koefisien untuk X~1~ adalah efek X~1~ ketika X~2~ = 0, yang tidak memiliki makna substantif
  * Dengan _centering_, koefisien X~1~ adalah efek X~1~ ketika X~2~ berada di rata-ratanya

* Selain itu, mencegah multikolinearitas antara prediktor dan suku interaksi 
  - Tanpa _centering_, variabel interaksi (X~1~ × X~2~) sangat berkorelasi dengan X~1~ dan X~2~ secara terpisah karena merupakan hasil kali keduanya 
  - _Centering_ tidak menghilangkan multikolinearitas secara substantif (yang alasannya _conceptual overlap_), tetapi hanya mengurangi **korelasi artifisial** antara _main effect_ dan suku interaksi
...
* Pada opsi **model builder**, klik **add new block**
  ...
  - Untuk membuat suku interaksi, sambil menekan tombol `ctrl`, klik **neu_mc** kemudian **trust_mc**, lalu klik tanda panah yang kedua dan pilih **interaction**
  - Masukkan **neu_mc** dan **trust_mc** sebagai prediktor individual untuk mengestimasi _main effect_
* Dengan begitu kita punya 2 model regresi
  - Model 1 prediktornya **hi**
  - Model 2 prediktornya **hi** dan interaksi antara **neu** dengan **trust** (_main effect_ + _interaction effect_)
* Pada opsi **assumption checks**, tambahkan centang pada **collinearity statistics**
* Pada opsi **model fit**, klik **AIC** dan **BIC**.
* Pada opsi **estimated marginal means**, masukkan **neu_mc** dan **trust_mc** dalam **terms 1**

## Model fit

![](libs/mfit-2.png){fig-align="center"}

Model 2 (_F_(4,395) = 74.3, _p_ = .001, _Adj. R_^2^ = .424) dapat menjelaskan varians tingkat kemandirian anak lebih baik daripada Model 1 (_F_(1,398) = 171.8, _p_ = .001, _R_^2^ = .300), dengan _overlapping variances_ sebesar 42.4% dibanding 30%.
...
Ketika dibandingkan, Model 1 dan Model 2 berbeda signifikan (_F_(3,395) = 29.5, _p_ = .001). ΔR^2^ = .128, artinya selisih _R_^2^ Model 1 dan Model 2 = .128 atau _R_^2^ naik sebesar 12.8%
...
Pendapatan keluarga dapat menjelaskan variasi kemandirian anak (_B_ = 0.153 95% CI [0.025, 0.281], _SE_ = 0.065, _t_ = 2.354, _p_ = .019). Artinya, anak yang dibesarkan di keluarga dengan tingkat pendapatan yang tinggi, maka cenderung lebih mandiri.
...
Tidak ada bukti bahwa ada interaksi antara _neuroticism_ dengan _trust in organismic development_ dalam menjelaskan varians kemandirian anak (_B_ = -0.0005 95% CI [-0.019, 0.018], _SE_ = 0.009, _t_ = -0.05, _p_ = .955).
...
* Interpretasi _slopes_ pada suku interaksi - bisa di-_probe_ dengan _estimated marginal means_
  - Dalam konteks analisis yang kita lakukan, suku interaksi nonsignifikan, sehingga kita melihat dua garis regresi yang sejajar, tidak berpotongan
...
* _Unstandardized B_ yang negatif, sebaliknya, ibu dengan kecenderungan _trust_ yang di atas rata-rata (+1SD), korelasi antara _neuroticism_ dengan kemandirian justru melemah/negatif
...
* Dapat dideteksi dengan melakukan analisis melihat _variance inflated factors_ (VIF)
  - Bila VIF \< 5, maka multi-kolinearitas tidak terjadi
* Penelitian _longitudinal_ punya potensi terjadinya autokorelasi (residual _time 1_ dan _time 2_ berkorelasi)
  - Dapat dicek dengan tes Durbin-Watson (nilai statistik DW berkisar 0–4; nilai ~2 artinya tidak ada autokorelasi, < 2 autokorelasi positif, > 2 autokorelasi negatif)
  - Beberapa perangkat lunak melaporkan nilai _p_ untuk tes ini; bila _p_ \> .05, autokorelasi residual tidak terjadi
  - Umumnya, untuk data _cross-sectional_, autokorelasi tidak terlalu relevan
...
> Ketika dibandingkan, Model 1 dan Model 2 berbeda signifikan (_F_(3,395) = 29.5, _p_ = .001). _R_^2^ bertambah sedikit (ΔR^2^ = .128). Model 2 (_F_(4,395) = 74.3, _p_ = .001, _Adj. R_^2^ = .424, AIC = 2472, BIC = 2496) dapat menjelaskan varians tingkat kemandirian anak sedikit lebih baik daripada Model 1 (_F_(1,398) = 171.8, _p_ = .001, _Adj. R_^2^ = .300, AIC = 2547, BIC = 2559).

## Bagaimana melaporkannya?

> Pendapatan keluarga dapat menjelaskan variasi kemandirian anak (_B_ = 0.153 95% CI [0.025, 0.281], _SE_ = 0.065, _t_ = 2.354, _p_ = .019). _Main effect neuroticism_ (_B_ = 0.387 95% CI [0.261, 0.514], _SE_ = 0.064, _t_ = 6.018, _p_ = .001) dan _trust in organismic development_ (_B_ = 0.391 95% CI [0.275, 0.508], _SE_ = 0.059, _t_ = 6.609, _p_ = .001) secara independen menjelaskan tingkat kemandirian anak. Tidak ada bukti bahwa interaksi antara _neuroticism_ dengan _trust in organismic development_ dapat menjelaskan varians kemandirian anak (_B_ = -0.0005 95% CI [-0.019, 0.018], _SE_ = 0.009, _t_ = -0.05, _p_ = .955).
...
### Dataset 2: [dataset-organisasi.omv](materials/dataset-organisasi.omv)
